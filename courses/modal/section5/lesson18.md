# Customizing the Coding Sandbox


We can customize the sandbox environment by defining any custom image and dependencies
we want to use. See `ModalSandbox.IMAGE`. We can also customize the sandbox by passing in
an `init_script` to run when the sandbox is created. This can define any libraries
and functions we may want to have predefined in the Python sandbox environment.
...
"""
sb = ModalSandbox(init_script=init_script)
sb.run_code("print(df.head())")
sb.run_code("print(greet('World'))")
sb.run_code("print(calculate_sum([1, 2, 3, 4, 5]))")
sb.run_code("print(square(5))")
sb.run_code("print(get_stats([1, 2, 3, 4, 5]))")
sb.terminate()
```



The code from the previous lesson is below for reference.
