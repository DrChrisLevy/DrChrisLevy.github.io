# Digging More into the Coding Sandbox

In this lessons we dig a little more deeper
into our `ModalSandbox` class to see how it works.

We are using the same code from the previous lesson.
...
How to use the `ModalSandbox` class:
Run each of these lines one at a time to see how it works.
...
sb2 = ModalSandbox(sandbox_id=sb_id) # same sandbox
sb2.run_code("print(df.sum())")
