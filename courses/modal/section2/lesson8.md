# Transcribing Audio Files with ASR model: `nvidia/parakeet-tdt-0.6b-v2` (Part 2)

- This is a continuation of the previous lesson where we add a few more features to the ASR model.
    - We add chunking to handle longer audio files.
    - We add a volume to save the results to.
    - We can debug intermediate chunks in the volume if necessary, with `keep_chunks=True`.

Note that we also make use of `starmap` to run the functions in parallel with multiple arguments.
You can read about the `starmap` documentation [here](https://modal.com/docs/guide/scale#starmap).

Put this code in a file called `asr_v2.py` and run it with `modal run asr_v2.py`.
...
image = (
    ...
    # .add_local_file("<other_source_file>", "/<other_source_file>")
)
